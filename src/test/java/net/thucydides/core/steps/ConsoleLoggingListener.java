...
public class ConsoleLoggingListener implements StepListener {
    ...
    public synchronized void testStarted(String description) {
        if (loggingLevelIsAtLeast(LoggingLevel.NORMAL)) {
    ...
    @Override
    public synchronized void testStarted(String description, String id) {
        if (loggingLevelIsAtLeast(LoggingLevel.NORMAL)) {
    ...
    public synchronized void testFinished(TestOutcome result) {
        if (result.isFailure()) {
            logFailure(result);
        }
        if (result.isError()) {
    ...
    private void logError(TestOutcome result) {
        Thread.dumpStack();
        System.out.println("XXX LogError");
        if (loggingLevelIsAtLeast(LoggingLevel.QUIET)) {
            getLogger().error(red(testFailureHeading() + "\nTEST FAILED WITH ERROR: {}"),
    ...
    private void logFailureCause(TestOutcome result) {
        if (result.getNestedTestFailureCause() != null) {
            ...
            for(StackTraceElement stackTrace: result.getNestedTestFailureCause().getOriginalCause().getStackTrace()){
                getLogger().error("XXX" +stackTrace);
            }
            getLogger().error("XXX" +result.getNestedTestFailureCause().getOriginalCause().getStackTrace());
